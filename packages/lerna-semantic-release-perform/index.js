...
function isPackageUpdated(pkg, npm, cb) {
  npm.getVersion(pkg.name)(function (err, publishedVersion) {
    ...
    log.info(
      ...
      outOfDate
        ? "has been updated, since version is newer than " + publishedVersion
        : "is up to date"
    );
...
function publishUpdatedPackages(updatedPackages, done) {
  ...
  const updatedPackageRelativeLocations = updatedPackageLocations.map(function (
    packageLocation
  ) {
    return {
      pkg: packageLocation.pkg,
      path: path.relative(io.shell.cwdSync(), packageLocation.path),
    };
  });
  ...
  async.series(
    updatedPackageRelativeLocations.map(function (packageRelativeLocation) {
